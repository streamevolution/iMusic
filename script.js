...
// const homeBtnSearch está eliminado
// const favoritesBtn está eliminado
...
// ⬇️ NUEVOS ELEMENTOS DE LA BARRA DE NAVEGACIÓN INFERIOR ⬇️
...
function handleDeletePlaylist(playlistName) {
    if (confirm(`¿Estás seguro de que quieres eliminar la playlist "${playlistName}"? Esta acción es irreversible.`)) {
        ...
        savePlaylists();
        alert(`Playlist "${playlistName}" eliminada correctamente.`);
        renderPlaylistMainModal(); // Recargar el modal principal
    }
}
...
function renderRenamePlaylistModal(oldName) {
    ...
    document.getElementById('saveRenameBtn').addEventListener('click', function saveRenameHandler() {
        ...
        if (newName !== oldName && userPlaylists[newName]) {
            alert(`Ya existe una playlist con el nombre "${newName}".`);
            return;
        }
        ...
        if (newName !== oldName) {
            ...
            savePlaylists();
            alert(`Playlist renombrada a "${newName}".`);
        }
        
        // Limpiar y volver a la vista principal de Playlists
        closeModalBtn.removeEventListener('click', modalCloseHandler);
        closeModal();
        renderPlaylistMainModal();
    });

    // 2. Manejar el clic en la 'X' (closeModalBtn) y el botón de retroceso para volver
    function modalCloseHandler() {
        // Remover este listener temporal para evitar duplicidad o problemas
        closeModalBtn.removeEventListener('click', modalCloseHandler); 
        // Cerrar el modal actual (el de renombrar)
        closeModal();
        // Volver a renderizar el modal principal de Playlists
        renderPlaylistMainModal();
    }
    ...
    function renderSongList() {
        ...
        document.querySelectorAll('.playlist-song-item').forEach(item => {
            ...
            item.querySelector('.delete-song-btn').addEventListener('click', (e) => {
                ...
                if (confirm(`¿Eliminar "${playlist[indexToDelete].title}" de la playlist "${playlistName}"?`)) {
                    userPlaylists[playlistName].splice(indexToDelete, 1);
                    savePlaylists();
                    alert("Canción eliminada.");
                    renderSongList(); // Recargar la lista después de eliminar
                }
            });
        });
...
function renderAddSongsToPlaylistModal(playlistName, isEditing = false) {
    ...
    finishBtn.addEventListener('click', () => {
        if (selectedSongsForNewPlaylist.length === 0) {
            if (!confirm(`La playlist "${playlistName}" quedará vacía. ¿Continuar?`)) {
                return;
            }
        }
